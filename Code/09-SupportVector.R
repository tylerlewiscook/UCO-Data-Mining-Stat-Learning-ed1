...
leuk <- read.csv(file="Data/leukemia.csv")
...
svmFit1 <- svm(tumor ~ ., data = datdim2, kernel = "linear", cost = 10, scale = FALSE)
plot(svmFit1, datdim2)
summary(svmFit1)

svmFit2 <- svm(tumor ~ ., data = datdim2, kernel = "linear", cost = 0.1, scale = FALSE)
...
svmFull <- svm(tumor ~ ., data = leuk, kernel = "linear", cost = 1, scale = FALSE)
plot(svmFull, leuk, gene1 ~ gene7)
fullPred <- predict(svmFull, leuk)
table(fullPred, leuk$tumor)

# Radial basis kernel
svmRad1 <- svm(tumor ~ ., data = datdim2, kernel = "radial", cost = 1, scale = FALSE)
...
svmRad2 <- svm(tumor ~ ., data = datdim2, kernel = "radial", cost = 5, scale = FALSE)
plot(svmRad2, datdim2)
radPred2 <- predict(svmRad2, datdim2)
table(radPred2, datdim2$tumor)


svmRad3 <- svm(tumor ~ ., data = datdim2, kernel = "radial", gamma = 1, cost = 5, scale = FALSE)
plot(svmRad3, datdim2)
radPred3 <- predict(svmRad3, datdim2)
table(radPred3, datdim2$tumor)


svmRad4 <- svm(tumor ~ ., data = datdim2, kernel = "radial", gamma = 5, cost = 5, scale = FALSE)
...
radCV <- tune(svm, tumor ~ ., data = datdim2, kernel = "radial", 
           ranges = list(cost = c(1, 5, 10, 50), gamma = c(0.25, 0.5, 1, 5)))
...
polyCV <- tune(svm, tumor ~ ., dat = leuk, kernel = "polynomial",
            ranges = list(cost = c(1, 5, 10), degree = c(2, 3, 4)))
